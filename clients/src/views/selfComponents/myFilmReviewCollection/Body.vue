...
<script>
  ...
  export default {
    ...
    methods: {
      getFilmReviews(){
        axios.get("/users/getFilmReviewCollection", {params : {user_id : this.$store.state._id}}).then((response) => {
          let res = response.data;
          if (res.status == '1') {
            this.filmReviews = res.result[0].filmReviewCollections;
          } else {
            this.filmReviews = [];
          }
        });
      ...
      getMore(){
        axios.get("/users/getFilmReviewCollectionByIndex", {params: { user_id : this.$store.state._id , index : this.filmReviews.length }}).then((response) => {
          let res = response.data;
          if (res.status == '1') {
            this.filmReviews = this.filmReviews.concat(res.result[0].filmReviewCollections);
            if(0<res.result[0].filmReviewCollections.length){
              console.log("获取影评成功，还有跟多");
            }else {
              this.moreFlag = 0;
            }
          } else {
